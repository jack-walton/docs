...
To use the link card shortcode, you need to set up an adapter in `src/astro.config.ts`. If you don’t need the link card shortcode, you can set `linkCard` to `false` in `src/config.ts`. In that case, you don’t need to configure the adapter before building.
